...
if yhdr.get("next_clicked"):
    ...
    st.rerun()
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# ANNO_DATI – KPI annuali (+ Δ YoY) con stessa grafica/struttura della striscia MESE
# Posizionare subito sotto la Striscia ANNO – Navigazione
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

import calendar  # (già importato in alto, resta harmless se duplicato)

def _fallback_rooms_avail_year(df_year_like: pd.DataFrame, default_rooms: int, rooms_map: dict[str,int]) -> float:
    """
    Calcola le camere disponibili nell'anno quando manca la colonna 'rooms_available'.
    Per ogni property presente nel df: camere_nominali * giorni_di_ogni_mese_presenti.
    """
    ...
    for p in props:
        ...
        for _, r in dfp[["year", "month"]].drop_duplicates().iterrows():
            ...
            tot += rooms * calendar.monthrange(y, m)[1]
    return float(tot)

def _sum_first(df: pd.DataFrame, candidates: list[str]) -> float:
...
def _mean_first(df: pd.DataFrame, candidates: list[str]) -> float:
    if df is None or df.empty:
        return 0.0
    for c in candidates:
        if c in df.columns:
...
# --- Aggregati ANNO corrente vs ANNO precedente ---
_df_y    = df_view[df_view["year"] == active_y].copy()
_df_y_py = df_view[df_view["year"] == active_y - 1].copy()

sold_y     = int(_sum_first(_df_y,    ["occupied","sold_nights","rooms_sold","nights","notti"]))
sold_y_py  = int(_sum_first(_df_y_py, ["occupied","sold_nights","rooms_sold","nights","notti"]))

rooms_y    = _sum_first(_df_y,    ["rooms_available","rooms_avail","camere_disponibili"])
rooms_y_py = _sum_first(_df_y_py, ["rooms_available","rooms_avail","camere_disponibili"])

if rooms_y == 0.0:
    rooms_y = _fallback_rooms_avail_year(_df_y, ROOMS_DEFAULT, ROOMS_MAP)
if rooms_y_py == 0.0:
    rooms_y_py = _fallback_rooms_avail_year(_df_y_py, ROOMS_DEFAULT, ROOMS_MAP)

occ_y    = (sold_y / rooms_y * 100.0) if rooms_y > 0 else 0.0
occ_y_py = (sold_y_py / rooms_y_py * 100.0) if rooms_y_py > 0 else 0.0

rev_y    = _sum_first(_df_y,    ["revenue","totale_revenue","ricavi"])
rev_y_py = _sum_first(_df_y_py, ["revenue","totale_revenue","ricavi"])

adr_y    = _mean_first(_df_y,    ["adr"])
adr_y_py = _mean_first(_df_y_py, ["adr"])

rpar_y    = _mean_first(_df_y,    ["revpar","rev_par"])
rpar_y_py = _mean_first(_df_y_py, ["revpar","rev_par"])

# --- Dizionari valore formattato + Δ YoY (assoluto) ---
kpi_year = {
    "Revenue anno": _fmt_eur(rev_y),
    "Occupazione anno": _fmt_pct(occ_y),
    "Notti vendute anno": _fmt_th(sold_y),
    "ADR medio anno": _fmt_eur(adr_y),
    "RevPAR medio anno": _fmt_eur(rpar_y),
}
delta_year_raw = {
    "Revenue anno": rev_y - rev_y_py,
    "Occupazione anno": occ_y - occ_y_py,          # punti percentuali
    "Notti vendute anno": float(sold_y - sold_y_py),
    "ADR medio anno": adr_y - adr_y_py,
    "RevPAR medio anno": rpar_y - rpar_y_py,
}

# --- Renderer UI: stesso layout della Mese_Dati (5 colonne ancorate 2|1|3|1|2) ---
def _pill_html(metric_key: str, delta_val: float) -> str:
    if delta_val is None:
        return ""
    up = (delta_val >= 0)
    cls = "up" if up else "down"
    icon = "↑" if up else "↓"
    # unità coerenti per ciascun KPI
    if "Occupazione" in metric_key:
        num = f"{abs(delta_val):,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
        txt = f"{icon} {num} pp"
    elif "Notti" in metric_key:
        num = f"{int(abs(delta_val))}"
        txt = f"{icon} {num}"
    else:
        # metriche monetarie (Revenue/ADR/RevPAR)
        sign = "-" if not up else ""
        txt = f"{icon} {sign}{_fmt_eur(abs(delta_val))}"
    return f'<span class="kpi-pill {cls}">{txt}</span>'

# Stili (riuso degli stessi token tipografici della striscia mese)
st.markdown("""
<style>
.yd-section{ width:100%; margin-top:4px; }
