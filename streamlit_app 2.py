...
, delta=f"{d_revpar:.2f}", delta_color="normal")
...
tbl = pd.DataFrame({
    ...
    'Revenue': [fmt_currency(curr['revenue']), fmt_currency(prev['revenue']), color_span(fmt_currency(d_rev), d_rev)],
    'Occupazione': [fmt_pct(curr['occ_pct']), fmt_pct(prev['occ_pct']), color_span(f"{d_occ:.2f}", d_occ, " pp")],
    'Notti': [curr['notti'], prev['notti'], color_span(f"{d_notti:+d}", d_notti)],
    'ADR': [fmt_currency(curr['adr']), fmt_currency(prev['adr']), color_span(fmt_currency(d_adr), d_adr)],
    'RevPAR': [fmt_currency(curr['revpar']), fmt_currency(prev['revpar']), color_span(fmt_currency(d_revpar), d_revpar)],
})
