...
def go_prev_year(): st.session_state["active_year"] -= 1
def go_next_year(): st.session_state["active_year"] += 1
...
# -----------------------------------------------------------------------------
# STRISCIA ANNO (stessa struttura/sizing della striscia MESE)
# -----------------------------------------------------------------------------
...
with cB:
    ...
    ynav = render_year_nav(active_y, key_prefix="ynav_main")
    st.markdown('</div>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

if ynav.get("prev_clicked"):
    go_prev_year(); st.rerun()
if ynav.get("next_clicked"):
    go_next_year(); st.rerun()

# -----------------------------------------------------------------------------
# KPI ANNO (sotto la striscia ANNO)
# -----------------------------------------------------------------------------
...
df_y    = df_view[df_view["year"] == active_y].copy()
df_y_py = df_view[df_view["year"] == active_y - 1].copy()
sold_y    = int(_sum_first(df_y,    ["notti","nights","rooms_sold","sold_nights","occupied"]))
sold_y_py = int(_sum_first(df_y_py, ["notti","nights","rooms_sold","sold_nights","occupied"]))

# Camere disponibili – fallback da config se necessario
...
rooms_y    = _sum_first(df_y,    ["rooms_available","camere_disponibili","rooms_avail"]) or _fallback_rooms_avail_year(df_y, ROOMS_DEFAULT)
rooms_y_py = _sum_first(df_y_py, ["rooms_available","camere_disponibili","rooms_avail"]) or _fallback_rooms_avail_year(df_y_py, ROOMS_DEFAULT)

occ_y    = (sold_y / rooms_y * 100.0) if rooms_y > 0 else 0.0
occ_y_py = (sold_y_py / rooms_y_py * 100.0) if rooms_y_py > 0 else 0.0
rev_y    = _sum_first(df_y,    ["revenue","ricavi","totale_revenue"])
rev_y_py = _sum_first(df_y_py, ["revenue","ricavi","totale_revenue"])
adr_y    = _mean_first(df_y,    ["adr"])
adr_y_py = _mean_first(df_y_py, ["adr"])
rpar_y   = _mean_first(df_y,    ["revpar"])
rpar_y_py= _mean_first(df_y_py, ["revpar"])
...
deltas_year = {
    "Revenue": rev_y - rev_y_py,
    "Occupazione": occ_y - occ_y_py,
    "Notti vendute": sold_y - sold_y_py,
    "ADR": adr_y - adr_y_py,
    "RevPAR": rpar_y - rpar_y_py,
}
render_year_bar(title_html="", kpi=kpi_year, deltas=deltas_year, key_prefix="ybar_main")

# -----------------------------------------------------------------------------
# STRISCIA MESE (stessa struttura/sizing della striscia ANNO)
# -----------------------------------------------------------------------------
st.markdown('<div class="dl-strip">', unsafe_allow_html=True)
...
# -----------------------------------------------------------------------------
# (segue tutto il resto della tua pagina: KPI mensili, tabelle, grafici...)
# -----------------------------------------------------------------------------
